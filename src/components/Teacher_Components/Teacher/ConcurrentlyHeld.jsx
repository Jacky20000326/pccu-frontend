...
import { getTeacherInfo, getAssistant, getConcurrentlyHeld } from "../../../Store/UploadTeacherSlice"

const ConcurrentlyHeld = () => {

    // get dispatch
    let dispatch = useDispatch()


    let getConcurrentlyHeldData = useSelector(item => item.UploadTeacherReducer.ConcurrentlyHeldData)

    const getAssistantInfo = async () => {
        await dispatch(getTeacherInfo())
        await dispatch(getConcurrentlyHeld())
    }
    ...
    return (
        <Teacher_assistant_container >
            <Teacher_title>兼任教師</Teacher_title>
            <Teacher_container>
                {
                    getConcurrentlyHeldData ? getConcurrentlyHeldData.map(data => {
                        return (
                            <Link to={`/department/teacher/${data.TR_name}`} >
                                <Teacher_getConcurrentlyHeld_container>
                                    <Teacher_getConcurrentlyHeld_image_container>
                                        <Teacher_image src={`${process.env.REACT_APP_API_URL_IMAGE}/${data.TR_image}`} />
                                        <Teacher_name>{data.TR_name}</Teacher_name>
                                    </Teacher_getConcurrentlyHeld_image_container>
                                    <Teacher_getConcurrentlyHeld_block_container>

                                        <Teacher_job_title>兼任教師</Teacher_job_title>
...
`
const Teacher_getConcurrentlyHeld_container = styled.div`
    display: flex;
...
`
const Teacher_getConcurrentlyHeld_image_container = styled.div`
      /* flex: 1.5; */
      width: 100%;
    position: relative;
    /* min-height: 300px; */
...
const Teacher_image = styled.img`
    ...
    object-fit: cover;
`
const Teacher_getConcurrentlyHeld_block_container = styled.div`
    flex: 5;
    padding: 20px;
    position: relative;
...
export default ConcurrentlyHeld
...
export const GetAllMember = createAsyncThunk(
    'GetAllMember',
    async () => {
        let result = await axios.get("http://140.137.51.13:3003/api/auth/getAllMember").then(res => res.data)
        return result.data
    }
)
...
export const DeletePermissions = createAsyncThunk(
    'Delete/permissions',
    async (data) => {
        let result = await axios.post("http://140.137.51.13:3003/api/auth/delete/permissions", data).then(res => res.data)
        return result
    }
)

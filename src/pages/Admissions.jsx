...
const Admissions = () => {
    ...
    const get_cuttent_route = (e) => {
        ...
        links[6].children_path.forEach(item => item.state = false)
        const findIndex = links[6].children_path.findIndex(item => item.path == e)
        if (findIndex) links[6].children_path[findIndex].state = true
        if (findIndex == 0) links[6].children_path[0].state = true
        setReload({})
    }

    const reload_path_compare = () => {
        links[6].children_path.forEach(item => item.state = false)
        const findIndex = links[6].children_path.findIndex(item => item.path == children_path)
    ...
    return (
        <Admission_container>
            <Admission_txt>招生訊息</Admission_txt>
            <Decorate_block />
            <Separate_line />
            <Admission_info_container>
                <Admission_links_container>
                    {
                        links[6].children_path.map(item =>
...
const Admission_container = styled.div`
...
`
const Admission_txt = styled.div`
	margin-top: 60px;
	color: #0050AE;
	font-size: 32px;
	@media (max-width: ${({ theme }) => theme.w_900.w}) {
...
`
const Admission_info_container = styled.div`
	display: flex;
	margin-top: 45px;
	@media (max-width: ${({ theme }) => theme.w_900.w}) {
...
`
const Admission_links_container = styled.div`
...
const Separate_line_rwd = styled.hr`
	...
	background-color: #555555;
`
const Admission_info = styled.div`
	margin-left: 70px;
	flex: 5;
	@media (max-width: ${({ theme }) => theme.w_900.w}) {
